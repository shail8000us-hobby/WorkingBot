...
6. **No cross-strategy bleed — SURGICAL bypass only (revised 2026-04-28).** `STRADDLE_WITH_ADJUSTMENT` bypasses two specific whipsaw behaviours: trigger widening (under-adjusting compounds straddle loss — [mmm_monitor.py:3550-3552](webui/backend/routes/mmm/mmm_monitor.py#L3550-L3552)) and lot reduction (under-hedging leaves both legs exposed — [mmm_monitor.py:5129-5131](webui/backend/routes/mmm/mmm_monitor.py#L5129-L5131)). It does **NOT** bypass the smart engine's `block` decision or token budget — those protect against runaway over-adjustment in choppy markets, which is desirable for a straddle (Phase 2 / D6, 2026-04-27). The original blanket bypass was over-applied; corrected by Phase 3 surgical fix at [mmm_whipsaw_smart.py:697-711](webui/backend/routes/mmm/mmm_whipsaw_smart.py#L697-L711). See `audit/mmm/coordination/01_straddle_whipsaw_bypass.md`.
