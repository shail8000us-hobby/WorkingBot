...
PARAM_RULES = {
    ...
    'smart_ws_late_session_relax_mins': {'type': int,  'min': 30,   'max': 420,  'hot': True},
    'smart_ws_token_refresh_per_hour':  {'type': float,'min': 0.0,  'max': 5.0,  'hot': True},
    ...
    'profit_ratchet_fresh_reset':        {'type': bool,  'min': None, 'max': None,  'hot': True},
    'ratchet_reset_cooldowns':           {'type': bool,  'min': None, 'max': None,  'hot': True},
    'ratchet_reset_whipsaw':             {'type': bool,  'min': None, 'max': None,  'hot': True},
    'ratchet_reset_regime_tier':         {'type': bool,  'min': None, 'max': None,  'hot': True},
    # ── Profit Target Exit ──
...
def get_param_info() -> Dict[str, Dict]:
    ...
    descriptions = {
        ...
        'smart_ws_late_session_relax_mins': 'Smart engine: when DTE (minutes) drops below this value, eased detector weights activate (less flip/ER sensitivity, more gamma weight). 30 < DTE < this value. Default 180 (last 3 hours).',
        'smart_ws_token_refresh_per_hour': 'Smart engine: tokens dripped back per hour of elapsed session time. Prevents budget starvation in long ODTE sessions. Default 1.0 (1 token/hour recovered).',
        ...
        'replenish_retry_max': 'Max fast-path retries within one replenish call (transient failures only). Default 3.',
        'replenish_retry_delay_sec': 'Seconds between retry attempts inside a single replenish call. Default 2.',
        'replenish_max_reprice_attempts': 'Max reprice cycles per smart_execute call during replenish. Default 2.',
        'replenish_shield_hold_secs': 'Seconds ATM shield is suppressed after replenish at a within-buffer strike. Default 900 (15 min).',
        # ATM Shield
        ...
        # Profit Ratchet Fresh-Reset
        'profit_ratchet_fresh_reset': 'MASTER SWITCH for fresh-algo reset at each profit milestone. Original profit_ratchet design intent: when a milestone is reached, the algo should evaluate the new market state without prejudice from the prior hours of accumulated guards/blocks/cooldowns. When ON, fires alongside the trigger-snapshot re-anchor; when OFF, ratchet behaves exactly as before. Independent sub-flags below let you enable each tier separately. NEVER clears: positions, P&L, fill cursor, monitor generation, paused/awaiting state, emergency flags, adjustment history. Default OFF — enable per session for testing.',
        'ratchet_reset_cooldowns': 'Tier A: clear cooldown/block counters at each milestone. Fields cleared: _shift_starv_count, _consecutive_same_dir_count, _consecutive_dir_blocked, _consecutive_reversal_skip_count, _cooldown_block_logged, _gamma_blocked_count, _delta_rescue_count, _adaptive_regime_candidate_beats, _trend_calm_beats, _trend_plateau_beats, _vol_regime_beats_below. Each is a pure counter/timer with verified safe reset (most are already reset elsewhere on natural events). Default ON when fresh-reset master is enabled. Hot-reloadable.',
        'ratchet_reset_whipsaw': 'Tier B: clear whipsaw engine state at each milestone (_whipsaw_score, _whipsaw_state, _whipsaw_skip_until, _whipsaw_last_checked_idx, _whipsaw_last_noise_at, plus all _smart_ws_* keys EXCEPT _smart_ws_series which is the source price-time buffer). Whipsaw engine re-builds its score/mode within 5–10 beats. Risk: in choppy market a whipsaw event during the rebuild window could fire one extra unblocked adjustment. Default OFF — explicit opt-in. Hot-reloadable.',
        'ratchet_reset_regime_tier': 'Tier C: reset trend/regime tier state at each milestone. Sets _trend_tier=0, _trend_regime=NORMAL, _trend_direction=none; clears _trend_ema/ema_prev, _regime_action, wind-down trigger flags, _atm_wind_down_triggered. Preserves _trend_anchor_spot deliberately — regime module re-derives tier/direction from market data + preserved anchor within 1–2 beats, so a real trend snaps back to the correct tier almost immediately. Risk: 1–2 beats of NORMAL tier before regime re-detects a genuine trend; tier-specific lot reductions/boosts are absent in that window. Default OFF — explicit opt-in. Hot-reloadable.',
        # Profit Target Exit
