    ...
    <Box>
      ...
      <Grid container spacing={3} mb={3}>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ 
            height: '100%', 
            background: margin?.zone === 'GREEN' ? 'rgba(76, 175, 80, 0.08)' : 
                       margin?.zone === 'YELLOW' ? 'rgba(255, 152, 0, 0.08)' : 
                       margin?.zone === 'ORANGE' ? 'rgba(255, 152, 0, 0.15)' : 
                       'rgba(244, 67, 54, 0.08)',
            border: `1px solid ${margin?.zone === 'GREEN' ? 'rgba(76, 175, 80, 0.3)' : 
                                 margin?.zone === 'YELLOW' ? 'rgba(255, 152, 0, 0.3)' : 
                                 margin?.zone === 'ORANGE' ? 'rgba(255, 152, 0, 0.5)' : 
                                 'rgba(244, 67, 54, 0.3)'}`
          }}>
            <CardContent>
              ...
              <LinearProgress 
                ...
                sx={{ 
                  ...
                  '& .MuiLinearProgress-bar': {
                    backgroundColor: margin?.zone === 'GREEN' ? '#4caf50' : margin?.zone === 'YELLOW' ? '#ff9800' : '#f44336'
                  }
                }}
              />
              ...
              <Box mt={2}>
                <Typography variant="body2" color="textSecondary">
                  Status: {margin?.can_open_positions ? '✅ Can open positions' : '⛔ REDUCE ONLY MODE'}
                </Typography>
                <Divider sx={{ my: 1 }} />
                <Typography variant="body2">Total Balance: {formatINR(margin?.total_balance)}</Typography>
                <Typography variant="body2">Available: {formatINR(margin?.available_balance)}</Typography>
                <Typography variant="body2">Blocked: {formatINR(margin?.blocked_balance)}</Typography>
              </Box>
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ 
            height: '100%', 
            background: distance?.zone === 'SAFE' ? 'rgba(76, 175, 80, 0.08)' : 
                       distance?.zone === 'ACCEPTABLE' ? 'rgba(33, 150, 243, 0.08)' : 
                       distance?.zone === 'CAUTION' ? 'rgba(255, 152, 0, 0.08)' : 
                       'rgba(244, 67, 54, 0.08)',
            border: `1px solid ${distance?.zone === 'SAFE' ? 'rgba(76, 175, 80, 0.3)' : 
                                 distance?.zone === 'ACCEPTABLE' ? 'rgba(33, 150, 243, 0.3)' : 
                                 distance?.zone === 'CAUTION' ? 'rgba(255, 152, 0, 0.3)' : 
                                 'rgba(244, 67, 54, 0.3)'}`
          }}>
            <CardContent>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
              ...
              <LinearProgress 
                ...
                sx={{ 
                  ...
                  backgroundColor: 'rgba(255, 255, 255, 0.1)',
                  '& .MuiLinearProgress-bar': {
                    backgroundColor: distance?.zone === 'SAFE' ? '#4caf50' : distance?.zone === 'ACCEPTABLE' ? '#2196f3' : distance?.zone === 'CAUTION' ? '#ff9800' : '#f44336'
                  }
                }}
              />
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ 
            height: '100%', 
            background: (mtm?.current_mtm_inr || 0) >= 0 ? 'rgba(76, 175, 80, 0.08)' : 'rgba(244, 67, 54, 0.08)',
            ...
            <CardContent>
              ...
              <Typography variant="h3" gutterBottom color={(mtm?.current_mtm_inr || 0) >= 0 ? 'success.main' : 'error.main'}>
              ...
              <Box mt={2}>
                ...
                <Divider sx={{ my: 1 }} />
                <Typography variant="body2">Unrealized P&L: {formatINR(margin?.unrealized_pnl || 0)}</Typography>
                <Typography variant="body2" sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 1 }}>
      ...
      {(margin?.zone !== 'GREEN' || distance?.zone !== 'SAFE') && (
        ...
        >
          <AlertTitle>
            {margin?.zone === 'RED' || distance?.zone === 'DANGER' ? '🚨 CRITICAL ALERT' : '⚠️ WARNING'}
          </AlertTitle>
          
          {margin?.zone === 'YELLOW' && (
            <Typography>
              <strong>REDUCE ONLY MODE</strong> - Margin utilization {formatPercent(margin?.utilization)} ≥ {formatPercent(apiConfig?.margin_utilization_max || 40)}. 
              No new BUY orders allowed. TP orders remain active.
            </Typography>
          )}
          
          {margin?.zone === 'ORANGE' && (
            <Typography>
              <strong>HIGH UTILIZATION</strong> - Add margin or reduce positions soon. Emergency reserve: {formatPercent(100 - (margin?.utilization || 0))}.
            </Typography>
          )}
          
          {margin?.zone === 'RED' && (
            <Typography>
              <strong>CRITICAL UTILIZATION</strong> - IMMEDIATE ACTION REQUIRED! Add margin NOW or close positions. Only {formatPercent(100 - (margin?.utilization || 0))} reserve remaining!
            </Typography>
          )}
          
          {distance?.zone === 'CAUTION' && (
            <Typography>
              <strong>LOW LIQUIDATION DISTANCE</strong> - Distance {formatPercent(distance?.distance)} below minimum {formatPercent(apiConfig?.liquidation_distance_min || 60)}. Add margin recommended.
            </Typography>
          )}
          
          {distance?.zone === 'DANGER' && (
            <Typography>
              <strong>CRITICAL LIQUIDATION RISK</strong> - Distance {formatPercent(distance?.distance)} is dangerously low! Add margin or close positions IMMEDIATELY!
            </Typography>
          )}
        </Alert>
      )}
      ...
      {/* Info Footer */}
      <Box mt={3} p={2} bgcolor="rgba(255, 255, 255, 0.05)" borderRadius={1} border="1px solid rgba(255, 255, 255, 0.1)">
        <Typography variant="body2" color="textSecondary" align="center">
          💡 <strong>Tip:</strong> Keep margin utilization below 40% to maintain 60% emergency reserve. 
          Monitor liquidation distance - below 60% is risky. 
          <br />
