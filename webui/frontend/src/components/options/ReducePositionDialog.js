...
import { TrendingDown as ReduceIcon } from '@mui/icons-material';

const MONO = 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace';
const AMBER = '#f59e0b';

function ReducePositionDialog({ open, rows, pct, onClose, onConfirm, executing }) {
    if (!rows || rows.length === 0) return null;

    const skipped = rows.filter(r => r.lotsToClose < 1);
        ...
        <Dialog
            ...
            PaperProps={{
                sx: {
                    ...
                    borderRadius: 2,
                    boxShadow: `0 0 40px ${alpha('#000', 0.7)}, 0 0 20px ${alpha(AMBER, 0.1)}`,
                },
            }}
        >
            {/* Header */}
            <DialogTitle sx={{ pb: 1, pt: 2, px: 2.5 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Box sx={{
                        ...
                        border: `1px solid ${alpha(AMBER, 0.35)}`,
                        flexShrink: 0,
                    }}>
                        <ReduceIcon sx={{ fontSize: 16, color: AMBER }} />
                    </Box>
                    <Box>
                        <Typography sx={{
                            color: AMBER,
            ...
            <DialogContent sx={{ px: 2.5, pb: 1 }}>
                {actionable.length > 0 && (
                    <Box sx={{ mt: 0.5 }}>
                        ...
                        <Box sx={{
                            ...
                            {['Symbol', 'Now', 'Action', 'Close', 'Left'].map((h, i) => (
                                <Typography key={h} sx={{
                                    fontSize: '0.6rem',
                                    fontWeight: 800,
                                    fontFamily: MONO,
                        ...
                        {actionable.map((r, idx) => (
                            <Box
                                ...
                                sx={{
                                    ...
                                    '&:hover': { bgcolor: alpha(AMBER, 0.06) },
                            ...
                            >
                                ...
                                <Typography sx={{
                                    ...
                                    fontFamily: MONO,
                                    fontWeight: 700,
                                    color: r.remaining === 0 ? '#f87171' : alpha('#4ade80', 0.85),
                                    textAlign: 'right',
                                }}>
                                    {r.remaining}
                                </Typography>
                ...
                {skipped.length > 0 && (
                    <Box sx={{
                        ...
                        <Typography sx={{ fontSize: '0.65rem', color: AMBER, fontWeight: 800, mt: 0.05 }}>⚠</Typography>
                        <Typography sx={{ fontSize: '0.65rem', color: alpha('#f59e0b', 0.85), fontFamily: MONO, lineHeight: 1.5 }}>
                            {skipped.length} skipped — {pct}% rounds to 0 lots:{' '}
                            <span style={{ color: alpha('#f59e0b', 0.65) }}>
                ...
                {actionable.length === 0 && (
                    <Box sx={{
                        ...
                        <Typography sx={{ fontSize: '0.72rem', color: alpha('#94a3b8', 0.8), fontFamily: MONO }}>
                            Nothing to reduce — {pct}% rounds to 0 lots for all selected strikes.
                        </Typography>
                    </Box>
                )}
            </DialogContent>
            ...
            <DialogActions sx={{ px: 2.5, pb: 2, pt: 1, gap: 1 }}>
                ...
                <Button
                    ...
                    sx={{
                        ...
                        bgcolor: AMBER,
                        color: '#0f172a',
                        boxShadow: `0 0 12px ${alpha(AMBER, 0.4)}`,
                        '&:hover': { bgcolor: '#fbbf24', boxShadow: `0 0 18px ${alpha(AMBER, 0.55)}` },
...
export default React.memo(ReducePositionDialog);
