...
const SortableRow = ({ pos, children }) => {
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: pos.product_symbol });
...
const OptionsPanel = () => {
  ...
  const [batchConfirmDialog, setBatchConfirmDialog] = useState({ open: false, orderCount: 0, estimatedTime: 0 });
  ...
  // Order types
  const ORDER_TYPES = {
    maker_first: { label: 'Smart (Maker First)', description: 'Try limit at mid-price, fallback to market' },
  ...
  const calculateSmartScaling = useCallback((position) => {
    const currentSize = Math.abs(position.size || 0);
    const pnlPct = position.pnl_percentage || 0;
    const positionDelta = position.greeks?.delta ? parseFloat(position.greeks.delta) * position.size : 0;
    ...
    switch (scalingStrategy) {
      ...
      case 'delta_neutral':
        ...
        if (deltaDeviation > scalingParams.deltaTolerance) {
          ...
          const helpsRebalance = (needsMoreLong && positionIsCall && positionIsLong) ||
    ...
    const handleKeyPress = (event) => {
      ...
      if (event.key === 'Escape') {
        if (closeDialog.open || addDialog.open) {
          ...
          setAddDialog({ open: false, position: null, size: lastUsedSize, side: 'sell', orderType: 'maker_first', limitPrice: '' });
  ...
  const calculateBatchOrders = () => {
    ...
    selectedPos.forEach(pos => {
      ...
      if (!Number.isInteger(buyRatio) || !Number.isInteger(sellRatio) || buyRatio <= 0 || sellRatio <= 0) {
        errors.push({
          symbol,
  ...
  const executeBatchOrders = async () => {
    ...
    if (orders.length === 0) {
      setOrderResult({ type: 'warning', message: 'No orders to execute. Enter quantities in Batch Qty column.' });
        ...
        try {
          ...
          if (data?.success) {
            const result = {
              ...
              message: executionMode === 'immediate' 
            ...
            if (errorMsg.toLowerCase().includes('rate') || errorMsg.toLowerCase().includes('too many')) {
          ...
          if (errorMsg.toLowerCase().includes('rate') || errorMsg.toLowerCase().includes('429') || errorMsg.toLowerCase().includes('too many')) {
            if (retries < MAX_RETRIES) {
              retries++;
              results[results.length - 1] = {
                symbol: order.symbol,
        ...
        try {
          ...
          if (data?.success && data.positions) {
            ...
            submittedOrders.forEach((orderResult, idx) => {
              ...
              const position = currentPositions.find(p => p.product_symbol === orderResult.symbol);
      ...
      const pollIntervalRef = setInterval(async () => {
        ...
        if (elapsedTime >= AUTO_MARKET_TIMEOUT && !autoMarketTriggered) {
          ...
          if (unfilledCount > 0) {
            ...
            activeIntervalsRef.current = activeIntervalsRef.current.filter(id => id !== pollIntervalRef);
        ...
        if (pollAttempts % 5 === 0) { // Update every 10 seconds
  ...
  return (
    ...
    >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              {hiddenPositions.length > 0 && (
                <Tooltip title="Show all hidden positions">
                  <Button size="small" color="secondary" variant="outlined" onClick={() => setHiddenPositions([])}>
          ...
          {uniqueExpiries.length > 1 && (
            <Box sx={{ mb: 2, display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {uniqueExpiries.map((expiry) => {
                ...
                const posCount = positions.filter(p => getExpiryCode(p.product_symbol) === expiry).length;
                return (
                  <Chip
                    key={expiry}
          ...
          <Box sx={{ mb: 2, p: 2, bgcolor: 'action.hover', borderRadius: 1 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  {indexPrices.BTC > 0 && (
                    <Box sx={{ 
                      ...
                      <Typography variant="caption" sx={{ color: '#3b82f6', fontWeight: 600, fontSize: '0.75rem', letterSpacing: 0.5 }}>
                        BTC SPOT
                      </Typography>
                      <Typography variant="h5" sx={{ color: '#3b82f6', fontWeight: 'bold', fontSize: '1.8rem', lineHeight: 1.1 }}>
                  ...
                  {indexPrices.ETH > 0 && (
                    <Box sx={{ 
                      ...
                      <Typography variant="caption" sx={{ color: '#a855f7', fontWeight: 600, fontSize: '0.75rem', letterSpacing: 0.5 }}>
                        ETH SPOT
                      </Typography>
                      <Typography variant="h5" sx={{ color: '#a855f7', fontWeight: 'bold', fontSize: '1.8rem', lineHeight: 1.1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {scalingStrategy === 'fixed' && (
                <Box>
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      const newParams = { ...scalingParams, stepSize: parseInt(e.target.value) || 5 };
                ...
                <>
                  <Box>
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        const newParams = { ...scalingParams, profitThreshold: parseFloat(e.target.value) || 10 };
                        setScalingParams(newParams);
                        localStorage.setItem('options_scaling_params', JSON.stringify(newParams));
                      }}
                      sx={{ width: 80 }}
                  ...
                  <Box>
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        const newParams = { ...scalingParams, lossThreshold: parseFloat(e.target.value) || -20 };
                        setScalingParams(newParams);
                        localStorage.setItem('options_scaling_params', JSON.stringify(newParams));
                      }}
                      sx={{ width: 80 }}
                ...
                <>
                  <Box>
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        const newParams = { ...scalingParams, deltaTarget: parseFloat(e.target.value) || 0 };
                        setScalingParams(newParams);
                        localStorage.setItem('options_scaling_params', JSON.stringify(newParams));
                      }}
                      sx={{ width: 80 }}
                  ...
                  <Box>
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        const newParams = { ...scalingParams, deltaTolerance: parseFloat(e.target.value) || 5 };
                        setScalingParams(newParams);
                        localStorage.setItem('options_scaling_params', JSON.stringify(newParams));
                      }}
                      sx={{ width: 80 }}
              ...
              <Box>
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    const newParams = { ...scalingParams, maxPositionSize: parseInt(e.target.value) || 50 };
                    setScalingParams(newParams);
                    localStorage.setItem('options_scaling_params', JSON.stringify(newParams));
                  }}
                  sx={{ width: 80 }}
            ...
            <Alert severity="info" sx={{ mt: 1.5 }}>
              <Typography variant="caption">
                {scalingStrategy === 'fixed' && `🔹 Fixed: Always add ${scalingParams.stepSize} contracts per click`}
                {scalingStrategy === 'profit_based' && `📈 Profit-Based: Scale into winners (>${scalingParams.profitThreshold}%), cautiously average down losers`}
                {scalingStrategy === 'delta_neutral' && `⚖️ Delta-Neutral: Auto-rebalance to maintain portfolio delta ~${scalingParams.deltaTarget}`}
              </Typography>
            </Alert>
          </Box>
          
          {/* Keyboard Shortcuts Legend */}
          <Box sx={{ mb: 2, p: 1, bgcolor: 'action.hover', borderRadius: 1, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell width="40px" padding="checkbox">
                      <Checkbox
                        indeterminate={Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length > 0 && Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length < sortedPositions.length}
                        checked={sortedPositions.length > 0 && Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length === sortedPositions.length}
                        onChange={(e) => e.target.checked ? selectAllStrikes() : deselectAllStrikes()}
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                        Symbol
                        {symbolSort === 'grouped' ? (
                          <Chip label="CE/PE" size="small" color="primary" sx={{ height: 18, fontSize: '0.65rem' }} />
                        ) : (
                  ...
                  >
                    <TableBody>
                      {sortedPositions.map((pos, index) => {
                        ...
                        const rowBgColor = isCall ? 'rgba(16, 185, 129, 0.12)' : isPut ? 'rgba(239, 68, 68, 0.12)' : 'transparent';
                        const rowHoverColor = isCall ? 'rgba(16, 185, 129, 0.2)' : isPut ? 'rgba(239, 68, 68, 0.2)' : 'action.hover';
                        ...
                        return (
                          <SortableRow key={pos.product_symbol} pos={pos}>
                                        ...
                                        <Tooltip title="Set Buy:Sell ratio to 1:2 (1 buy + 2 sell per multiplier)">
                                          <Chip
                                            ...
                                            sx={{
                                              ...
                                              height: '20px',
                                              cursor: 'pointer',
                                              bgcolor: strikeRatios[pos.product_symbol]?.buy === 1 && strikeRatios[pos.product_symbol]?.sell === 2 
                                                ? 'rgba(59, 130, 246, 0.5)' 
                                        ...
                                        <Tooltip title="Set Buy:Sell ratio to 1:1 (equal quantities)">
                                          <Chip
                                            ...
                                            sx={{
                                              ...
                                              height: '20px',
                                              cursor: 'pointer',
                                              bgcolor: strikeRatios[pos.product_symbol]?.buy === 1 && strikeRatios[pos.product_symbol]?.sell === 1 
                                                ? 'rgba(59, 130, 246, 0.5)' 
                                                : 'rgba(255,255,255,0.1)',
                                              '&:hover': { bgcolor: 'rgba(59, 130, 246, 0.3)' }
                                            }}
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Tooltip title={hiddenPositions.includes(pos.product_symbol) ? "Unhide position" : "Hide position (focus mode)"}>
                                <IconButton
                                  size="small"
                                  color={hiddenPositions.includes(pos.product_symbol) ? "success" : "default"}
                                  onClick={() => {
                                ...
                                >
                                  {hiddenPositions.includes(pos.product_symbol) ? <CheckCircleIcon /> : <BlockIcon />}
                                </IconButton>
                              </Tooltip>
                              <Chip 
                          ...
                          <TableCell align="right" sx={cellSx}>
                            <Tooltip title={`${daysToExp.toFixed(1)} days to expiry`}>
                              <Chip 
                                ...
                                icon={<TimerIcon />}
                                sx={{
                                  borderColor: daysToExp < 1 ? '#ef4444' : daysToExp < 7 ? '#f59e0b' : undefined,
                                  color: daysToExp < 1 ? '#ef4444' : daysToExp < 7 ? '#f59e0b' : undefined
                                }}
                              />
                            </Tooltip>
                          </TableCell>
                            ...
                            <TextField
                              ...
                              sx={{
                                ...
                                '& .MuiInputBase-input': {
                                  ...
                                  fontSize: '0.875rem',
                                  padding: '4px 8px',
                                  color: batchQuantities[pos.product_symbol] > 0 ? '#10b981' : batchQuantities[pos.product_symbol] < 0 ? '#ef4444' : 'inherit'
                                }
                              }}
                            />
                          </TableCell>
                                ...
                                const tooltipTitle = (
                                  <Box>
                                    ...
                                    <Typography variant="caption" sx={{ display: 'block', mb: 0.5 }}>
                                      Action: {recommendation.action === 'scale' ? '✅ Scale In' : recommendation.action === 'reduce' ? '⚠️ Reduce' : '⏸️ Hold'}
                                    </Typography>
                                    ...
                                    <Typography variant="caption" sx={{ display: 'block', color: recommendation.riskLevel === 'high' ? '#ef4444' : recommendation.riskLevel === 'low' ? '#10b981' : '#f59e0b' }}>
                                ...
                                return (
                                  <Tooltip title={tooltipTitle}>
                                    <IconButton 
                                      ...
                                      sx={{ 
                                        bgcolor: recommendation.action === 'scale' ? (isCall ? '#10b981' : '#ef4444') : 
                                                 recommendation.action === 'reduce' ? '#ef4444' : '#6b7280',
                                        color: 'white',
                                        fontWeight: 'bold',
                                        fontSize: 14,
                                        minWidth: 36,
                                        '&:hover': { bgcolor: recommendation.action === 'scale' ? (isCall ? '#059669' : '#dc2626') : '#4b5563' },
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Chip 
                    ...
                    size="small"
                    color={Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length > 0 ? 'primary' : 'default'}
                  />
                  {Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length > 0 && (
                    <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                      <Typography variant="caption" color="text.secondary">Set All Ratio:</Typography>
                      <Button
                        size="small"
                        variant="outlined"
                        onClick={() => {
                          const selected = Object.keys(selectedStrikes).filter(k => selectedStrikes[k]);
                          const newRatios = {};
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Typography variant="body2" color="text.secondary">Quantity (lots)</Typography>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <IconButton 
                      size="small" 
                      onClick={() => setOrderQuantity(q => q > 0 ? Math.max(1, q - 1) : Math.max(-10, q - 1))}
                      sx={{ 
                    ...
                    <IconButton 
                      size="small" 
                      onClick={() => setOrderQuantity(q => q >= 0 ? Math.min(10, q + 1) : Math.min(-1, q + 1))}
                      sx={{ 
                        bgcolor: 'rgba(255,255,255,0.1)', 
                        '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' }
                      }}
                    ...
                    <Box sx={{ display: 'flex', borderRadius: 1, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.2)' }}>
                      <Button
                        ...
                        sx={{
                          borderRadius: 0,
                          minWidth: 80,
                          bgcolor: executionMode === 'immediate' ? 'rgba(239, 68, 68, 0.8)' : 'transparent',
                          color: executionMode === 'immediate' ? '#fff' : 'rgba(239, 68, 68, 0.8)',
                          borderColor: 'transparent',
                          '&:hover': {
                            bgcolor: executionMode === 'immediate' ? 'rgba(239, 68, 68, 1)' : 'rgba(239, 68, 68, 0.1)',
                            borderColor: 'transparent'
                          }
                        }}
                      >
                      ...
                      <Button
                        ...
                        sx={{
                          borderRadius: 0,
                          minWidth: 80,
                          bgcolor: executionMode === 'smart' ? 'rgba(16, 185, 129, 0.8)' : 'transparent',
                          color: executionMode === 'smart' ? '#fff' : 'rgba(16, 185, 129, 0.8)',
                          borderColor: 'transparent',
                          '&:hover': {
                            bgcolor: executionMode === 'smart' ? 'rgba(16, 185, 129, 1)' : 'rgba(16, 185, 129, 0.1)',
                            borderColor: 'transparent'
                          }
                        }}
                      >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Button
                    ...
                    startIcon={batchExecuting ? <CircularProgress size={16} color="inherit" /> : <PlayArrowIcon />}
                    ...
                    sx={{ minWidth: 150 }}
                  >
                    {batchExecuting ? 'Executing...' : `Execute ${calculateBatchOrders().length} Orders`}
                  </Button>
                </Box>
              </Box>
              ...
              {Object.keys(selectedStrikes).filter(k => selectedStrikes[k]).length > 0 && (
                <Box sx={{ mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                  <Typography variant="caption" color="text.secondary" sx={{ mb: 1, display: 'block' }}>
                    Order Preview ({executionMode === 'immediate' ? 'Market Orders - Instant Fill' : 'Limit Orders @ Mid - Auto-Market after 5min'}):
                  </Typography>
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                    {calculateBatchOrders().map((order, idx) => (
                      <Chip
                        ...
                        sx={{
                          bgcolor: order.side === 'buy' ? 'rgba(16, 185, 129, 0.2)' : 'rgba(239, 68, 68, 0.2)',
                ...
                <Box sx={{ mt: 2, pt: 2, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    {batchOrderResults.map((result, idx) => {
                      ...
                      const color = result.filled ? '#10b981' : result.success ? '#3b82f6' : '#ef4444';
                      ...
                      return (
                        ...
                        >
                          <span>{icon}</span>
                          <span>{result.symbol.split('-').slice(0, 3).join('-')}: {result.message}</span>
                        </Typography>
                      );
                    })}
                  </Box>
            ...
            <>
              <Box sx={{ mt: 2, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                <Chip 
                  ...
                  sx={{ 
                    bgcolor: getPnlColor(sortedPositions.reduce((sum, p) => sum + (p.unrealized_pnl || 0), 0)) + '20',
                    color: getPnlColor(sortedPositions.reduce((sum, p) => sum + (p.unrealized_pnl || 0), 0))
                  }}
                />
                <Chip 
                    ...
                    <Box sx={{ mb: 2, p: 1.5, bgcolor: 'background.paper', borderRadius: 1, border: '1px solid', borderColor: 'divider' }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                        ...
                        {aggregatedGreeks.btcDelta !== 0 && (
                          <Tooltip title={`Your BTC options have the same directional exposure as ${Math.abs(Number(aggregatedGreeks.btcDelta) || 0).toFixed(4)} BTC futures contracts`}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Typography 
                                variant="body2" 
                                fontWeight="bold"
                                sx={{ color: aggregatedGreeks.btcDelta >= 0 ? '#10b981' : '#ef4444' }}
                              >
                                {aggregatedGreeks.btcDelta >= 0 ? 'Long' : 'Short'} {Math.abs(Number(aggregatedGreeks.btcDelta) || 0).toFixed(4)} BTC
                              </Typography>
                              {aggregatedGreeks.btcDelta >= 0 ? (
                        ...
                        {aggregatedGreeks.ethDelta !== 0 && (
                          <Tooltip title={`Your ETH options have the same directional exposure as ${Math.abs(Number(aggregatedGreeks.ethDelta) || 0).toFixed(4)} ETH futures contracts`}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Typography 
                                variant="body2" 
                                fontWeight="bold"
                                sx={{ color: aggregatedGreeks.ethDelta >= 0 ? '#10b981' : '#ef4444' }}
                              >
                                {aggregatedGreeks.ethDelta >= 0 ? 'Long' : 'Short'} {Math.abs(Number(aggregatedGreeks.ethDelta) || 0).toFixed(4)} ETH
                              </Typography>
                              {aggregatedGreeks.ethDelta >= 0 ? (
                                <TrendingUp sx={{ fontSize: 16, color: '#10b981' }} />
                              ) : (
                      ...
                      {Math.abs(aggregatedGreeks.delta) < 0.1 && (
                        <Alert severity="info" sx={{ mt: 1.5 }} icon={<CheckCircleIcon />}>
                          <Typography variant="caption">
                            ✅ Portfolio is <strong>Delta Neutral</strong> (delta ≈ 0). Minimal directional exposure.
                          </Typography>
                        </Alert>
                      )}
                      ...
                      {Math.abs(aggregatedGreeks.delta) > 10 && (
                        <Alert severity="warning" sx={{ mt: 1.5 }} icon={<WarningIcon />}>
                          <Typography variant="caption">
                            ⚠️ High directional exposure. Consider hedging with {aggregatedGreeks.delta > 0 ? 'short' : 'long'} futures.
                          </Typography>
                        </Alert>
                      )}
                    </Box>
      ...
      <Dialog 
        open={addDialog.open} 
        onClose={() => setAddDialog({ open: false, position: null, size: lastUsedSize, side: 'sell', orderType: 'maker_first', limitPrice: '' })} 
        maxWidth="sm" 
        fullWidth
        disableRestoreFocus
      >
        ...
        <DialogContent>
          ...
          {addDialog.recommendation && addDialog.recommendation.action !== 'hold' && (
            <Alert 
              severity={
                addDialog.recommendation.action === 'scale' && addDialog.recommendation.riskLevel === 'low' ? 'success' :
                addDialog.recommendation.action === 'scale' && addDialog.recommendation.riskLevel === 'medium' ? 'info' :
                addDialog.recommendation.action === 'reduce' ? 'error' : 'warning'
              }
              sx={{ mb: 2 }}
            >
              <Typography variant="body2" sx={{ fontWeight: 'bold', mb: 0.5 }}>
                {addDialog.recommendation.action === 'scale' ? '✅ Recommended: Scale In' : '⚠️ Recommended: Reduce'}
              </Typography>
              <Typography variant="caption" sx={{ display: 'block', mb: 0.5 }}>
              ...
              <Typography variant="caption" sx={{ display: 'block', fontStyle: 'italic' }}>
                Strategy: {scalingStrategy.replace('_', ' ').toUpperCase()} | 
                Risk: {addDialog.recommendation.riskLevel.toUpperCase()} | 
                Confidence: {addDialog.recommendation.confidence.toUpperCase()}
              </Typography>
            </Alert>
          )}
          
          <Typography sx={{ mb: 2 }}>
            Add to your position in{' '}
            <strong>{addDialog.position?.product_symbol}</strong>
          </Typography>
        ...
        <DialogActions sx={{ justifyContent: 'space-between', px: 3, pb: 2 }}>
          <Button onClick={() => setAddDialog({ open: false, position: null, size: lastUsedSize, side: 'sell', orderType: 'maker_first', limitPrice: '' })}>
            Cancel (Esc)
          </Button>
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button 
            ...
            <Button 
              ...
              disabled={submittingOrder || !addDialog.size || parseFloat(addDialog.size) <= 0}
            >
              {submittingOrder ? 'Submitting...' : `${addDialog.side === 'buy' ? 'Buy' : 'Sell'} ${addDialog.size || 0}`}
            </Button>
          </Box>
        </DialogActions>
      </Dialog>
