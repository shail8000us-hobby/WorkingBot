      ...
      <CardContent>
        ...
        {loading ? (
          ...
          <Alert severity="info" icon={<Psychology />}>
            ...
            <Button variant="text" size="small" onClick={handleAnalyze} sx={{ mt: 1 }}>
          ...
          <>
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
              <Grid item xs={6} sm={4}>
                <Paper sx={{ p: 1.5, textAlign: 'center', bgcolor: 'action.hover' }}>
                  ...
                  <Typography
                    ...
                    fontWeight="bold"
                    color={
                      profile.risk_appetite > 0.6
                        ? 'error.main'
                        : profile.risk_appetite < 0.4
                          ? 'success.main'
                          : 'warning.main'
                    }
                  >
              ...
              <Grid item xs={6} sm={4}>
                <Paper sx={{ p: 1.5, textAlign: 'center', bgcolor: 'action.hover' }}>
                  ...
                  <Typography
                    ...
                    fontWeight="bold"
                    color={
                      profile.trend_preference === 'bullish'
                        ? 'success.main'
                        : profile.trend_preference === 'bearish'
                          ? 'error.main'
                          : 'info.main'
                    }
                  >
                    {(profile.trend_preference || 'Neutral').charAt(0).toUpperCase() +
                      (profile.trend_preference || 'neutral').slice(1)}
                  </Typography>
                </Paper>
              </Grid>
              <Grid item xs={6} sm={4}>
                <Paper sx={{ p: 1.5, textAlign: 'center', bgcolor: 'action.hover' }}>
                  ...
                  <Typography
                    variant="h6"
                    fontWeight="bold"
                    color={(profile.win_rate || 0) > 0.5 ? 'success.main' : 'error.main'}
                  >
              ...
              <Grid item xs={6} sm={4}>
                <Paper sx={{ p: 1.5, textAlign: 'center', bgcolor: 'action.hover' }}>
                  ...
                  <Typography variant="h6" fontWeight="bold">
                    {(profile.scaling_behavior || 'All In')
                      .replace(/_/g, ' ')
                      .split(' ')
                ...
                <Paper sx={{ p: 2, bgcolor: 'action.hover', mt: 1 }}>
                  <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body2" fontWeight="bold">
                        {profile.patience_factor > 0.6
                          ? 'Patient'
                          : profile.patience_factor < 0.4
                            ? 'Reactive'
                            : 'Moderate'}
                      </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body2" fontWeight="bold">
                        {(profile.volatility_preference || 'Medium').charAt(0).toUpperCase() +
                          (profile.volatility_preference || 'medium').slice(1)}
                      </Typography>
                    </Grid>
                  </Grid>
                </Paper>
            ...
            {summary?.strengths && summary.strengths.length > 0 && (
              <Box mb={2}>
                ...
                >
                  <Typography variant="subtitle1" fontWeight="bold">
                    <EmojiEvents
                      sx={{ fontSize: 18, mr: 1, verticalAlign: 'middle', color: 'success.main' }}
                    />
                    Strengths
                  </Typography>
            ...
            {summary?.areas_to_improve && summary.areas_to_improve.length > 0 && (
              <Box mb={2}>
                ...
                >
                  <Typography variant="subtitle1" fontWeight="bold">
                    <Warning
                      sx={{ fontSize: 18, mr: 1, verticalAlign: 'middle', color: 'warning.main' }}
                    />
                    Areas to Improve
                  </Typography>
            ...
            <Typography variant="caption" color="text.secondary" display="block" textAlign="right">
              Based on {profile.total_trades_analyzed || 0} trades • Analyzed:{' '}
