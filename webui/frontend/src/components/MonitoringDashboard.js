  ...
  return (
    <Box sx={{ p: 0 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1.2fr' }, gap: 2 }}>
            ...
            {priceHealth && !priceHealth.error ? (
              <>
                <Box sx={{ textAlign: 'center', mb: 2 }}>
                  <Chip
                    icon={priceHealth.is_fresh || priceHealth.fresh ? <CheckCircle /> : <Warning />}
                    label={priceHealth.status || (priceHealth.is_fresh || priceHealth.fresh ? 'Fresh' : 'Stale')}
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Typography variant="body2" fontWeight={600}>
                      {priceHealth.price_age_seconds?.toFixed(1) || priceHealth.age_seconds?.toFixed(1) || 'N/A'}s
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            {tpVerification && !tpVerification.error ? (
              <>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography variant="body2" color="warning.main">⚠ Orphaned:</Typography>
                    <Typography variant="body2" fontWeight={600}>
        ...
        <Card sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', display: 'flex', flexDirection: 'column', height: { xs: 'auto', lg: '100%' } }}>
          <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {advancedPredictions && !advancedPredictions.error ? (
              <>
                  ...
                  <Box sx={{ mb: 2 }}>
                    {advancedPredictions.warnings.map((warning, idx) => (
                      ...
                      >
                        ...
                        <Typography variant="caption" display="block" sx={{ mt: 0.5, fontWeight: 600, color: 'text.primary' }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-around', mb: 2, p: 1.5, bgcolor: 'action.hover', borderRadius: 1 }}>
                  ...
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="body1" fontWeight={700}>
                      {advancedPredictions.current_state?.positions || 0}/{advancedPredictions.current_state?.max_positions || 10}
                    </Typography>
                  </Box>
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="caption" color="text.secondary">Grid Step</Typography>
                ...
                {advancedPredictions.next_action && (
                  <Alert 
                    severity={
                      advancedPredictions.next_action.type.includes('PENDING') ? 'info' :
                      advancedPredictions.next_action.type.includes('WILL') ? 'success' :
                      advancedPredictions.next_action.type === 'CAPACITY_FULL' ? 'warning' :
                      'default'
                    }
                    icon={
                      advancedPredictions.next_action.type.includes('BUY') ? <TrendingDown /> :
                      advancedPredictions.next_action.type.includes('SELL') ? <TrendingUp /> :
                      <Speed />
                    }
                    sx={{ mb: 2 }}
                  >
                    ...
                    <Typography variant="caption" display="block" fontWeight={600} sx={{ mt: 1, color: 'text.primary' }}>
                ...
                <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, mt: 1, color: 'text.primary' }}>
                ...
                <Box sx={{ flex: 1, overflowY: 'auto', pr: 1 }}>
                  ...
                  {advancedPredictions.scenarios?.if_pending_fills && (
                    <Card sx={{ mb: 2, bgcolor: 'success.dark', border: '2px solid', borderColor: 'success.main' }}>
                      <CardContent sx={{ py: 1.5 }}>
                        <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, color: 'success.light' }}>
                          💚 Scenario 1: If Pending Order Fills
                        </Typography>
                        {advancedPredictions.scenarios.if_pending_fills.actions?.map((action, idx) => (
                          <Box 
                            ...
                            sx={{ 
                              ...
                              borderRadius: 1,
                              borderLeft: 3,
                              borderColor: action.action === 'PLACE_TP' ? 'info.main' : 'success.light'
                            }}
                          >
                            <Chip 
                              label={`#${action.sequence}`} 
                              size="small" 
                              sx={{ bgcolor: 'success.main', color: 'white', fontWeight: 700, minWidth: 35 }}
                            />
                            <Box sx={{ flex: 1 }}>
                              ...
                              {action.reason && (
                                <Typography variant="caption" display="block" sx={{ color: 'grey.300' }}>
                                  {action.reason}
                                </Typography>
                              )}
                            </Box>
                  ...
                  {advancedPredictions.scenarios?.if_tp_fills && (
                    <Card sx={{ mb: 2, bgcolor: 'info.dark', border: '2px solid', borderColor: 'info.main' }}>
                      <CardContent sx={{ py: 1.5 }}>
                        <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, color: 'info.light' }}>
                        ...
                        {advancedPredictions.scenarios.if_tp_fills.actions?.map((action, idx) => (
                          <Box 
                            ...
                            sx={{ 
                              ...
                              borderRadius: 1,
                              borderLeft: 3,
                              borderColor: action.action === 'CANCEL_PENDING_BUY' ? 'warning.main' : 'info.light'
                            }}
                          >
                            <Chip 
                              label={`#${action.sequence}`} 
                              size="small" 
                              sx={{ bgcolor: 'info.main', color: 'white', fontWeight: 700, minWidth: 35 }}
                            />
                            <Box sx={{ flex: 1 }}>
                              <Typography variant="body2" fontWeight={600} sx={{ color: 'white' }}>
                                {action.action.replace(/_/g, ' ')}
                        ...
                        <Alert severity="success" sx={{ mt: 1, fontSize: '0.75rem' }}>
                          <Typography variant="caption" fontWeight={700}>
                            Profit Realized: +${advancedPredictions.scenarios.if_tp_fills.profit_realized?.toLocaleString() || 'N/A'}
                          </Typography>
                        </Alert>
                      </CardContent>
                    </Card>
                  ...
                  {advancedPredictions.full_sequence?.sequence && (
                    <Card sx={{ mb: 1, bgcolor: 'background.default', border: '1px dashed', borderColor: 'divider' }}>
                      <CardContent sx={{ py: 1.5 }}>
              ...
              <>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-around', mb: 2, p: 1, bgcolor: 'action.hover', borderRadius: 1 }}>
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="caption" color="text.secondary">Price</Typography>
                    <Typography variant="body1" fontWeight={700} color="primary.main">
                      ${(predictiveMap.current_state?.price || predictiveMap.current_price)?.toLocaleString() || 'N/A'}
                    </Typography>
                  </Box>
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="caption" color="text.secondary">Mode</Typography>
                  ...
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="body1" fontWeight={700}>
                      {predictiveMap.current_state?.positions !== undefined && predictiveMap.current_state?.max_positions
                  ...
                  return (
                    <>
                      ...
                      {executedOrders.length > 0 && (
                        <Box sx={{ mb: 2, p: 1.5, bgcolor: 'success.dark', borderRadius: 1, border: '1px solid', borderColor: 'success.main' }}>
                          <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, color: 'success.light' }}>
                            📍 Executed Orders (Bot Memory)
                          </Typography>
                          {executedOrders.map((order, idx) => (
                            <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, borderBottom: idx < executedOrders.length - 1 ? '1px solid' : 'none', borderColor: 'success.main' }}>
                              <Typography variant="caption" sx={{ color: 'success.light' }}>
                      ...
                      {placedTPs.length > 0 && (
                        <Box sx={{ mb: 2, p: 1.5, bgcolor: 'info.dark', borderRadius: 1, border: '1px solid', borderColor: 'info.main' }}>
                          <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1, color: 'info.light' }}>
                            📤 Placed TPs (Bot Memory)
                          </Typography>
                          {placedTPs.map((tp, idx) => (
                            <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, borderBottom: idx < placedTPs.length - 1 ? '1px solid' : 'none', borderColor: 'info.main' }}>
                              <Typography variant="caption" sx={{ color: 'info.light' }}>
                                TP @ ${tp.tp_price?.toLocaleString()} (Entry: ${tp.entry_price?.toLocaleString()})
                              </Typography>
                              <Typography variant="caption" fontWeight={600} sx={{ color: 'white' }}>
                                Size: {tp.size}
                              </Typography>
                      ...
                      {nextAction && (
                        <Alert 
                          severity={
                            nextAction.type.includes('PENDING') ? 'info' :
                            nextAction.type === 'CAPACITY_FULL' ? 'warning' :
                            'success'
                          }
                          sx={{ mb: 2, fontSize: '0.875rem' }}
                        >
                          <Typography variant="body2" fontWeight={600}>
                            {nextAction.type === 'PENDING_BUY' ? '⏳ Pending BUY Order' :
                             nextAction.type === 'PENDING_SELL' ? '⏳ Pending SELL Order' :
                             nextAction.type === 'PENDING_TP' ? '⏳ Pending TP Order' :
                             nextAction.type === 'WILL_BUY' ? '🎯 Next Action: BUY' :
                             nextAction.type === 'WILL_SELL' ? '🎯 Next Action: SELL' :
                             nextAction.type === 'CAPACITY_FULL' ? '⚠️ Capacity Full' :
                             '⏸️ No Action'}
                          </Typography>
                          <Typography variant="caption" display="block" sx={{ mt: 0.5 }}>
                            {nextAction.price && `@ $${nextAction.price.toLocaleString()}`} {nextAction.status}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                          ...
                          {levels.map((level, idx) => (
                            <Box 
                              ...
                              sx={{ 
                                ...
                                borderLeft: 3,
                                borderColor: level.action?.toLowerCase().includes('buy') ? 'success.main' : 
                                           level.action?.toLowerCase().includes('tp') ? 'info.main' :
                                           level.action?.toLowerCase().includes('sell') ? 'error.main' : 'grey.500'
                              }}
                            >
                              <Chip 
                                label={level.action} 
                                size="small"
                                color={
                                  level.action?.toLowerCase().includes('buy') ? 'success' :
                                  level.action?.toLowerCase().includes('tp') ? 'info' :
                                  level.action?.toLowerCase().includes('sell') ? 'error' :
                                  'default'
                                }
                              ...
                              <Box sx={{ flex: 1 }}>
                                <Typography variant="caption" color="text.secondary" display="block">
                                  {level.reason || (level.change_pct ? `${level.change_pct.toFixed(2)}%` : '')}
                                </Typography>
                                {level.tp_price && (
                                  <Typography variant="caption" color="success.main" display="block">
                                    TP: ${level.tp_price.toLocaleString()} (+${level.profit_target?.toFixed(0)})
                                  </Typography>
                                )}
                                {level.entry_price && (
                                  <Typography variant="caption" color="text.secondary" display="block">
                                    Entry: ${level.entry_price.toLocaleString()} | Profit: +${level.profit?.toFixed(0)}
                                  </Typography>
                                )}
                              </Box>
                              {level.sequence && (
                                <Chip label={`#${level.sequence}`} size="small" sx={{ minWidth: 40 }} />
                              )}
                            </Box>
                          ))}
                        </Box>
