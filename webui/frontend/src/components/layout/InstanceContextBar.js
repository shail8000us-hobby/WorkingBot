...
function InstanceContextBar({ gridInfo, status, pnl }) {
  ...
  return (
    ...
    >
          ...
          {Object.entries(instancesBySymbol).map(([sym, insts], idx) => (
            <Box key={sym}>
              ...
              <Box sx={{ px: 2, py: 0.5 }}>
                <Typography
                  variant="caption"
                  sx={{ color: getSymbolColor(sym).primary, fontWeight: 600 }}
                >
                  {sym}
                </Typography>
              </Box>
                ...
                return (
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%' }}>
                      ...
                      <Typography sx={{ flex: 1 }}>{inst.name}</Typography>
                      {!inst.enabled && (
                        <Typography variant="caption" color="text.secondary">
                          disabled
                        </Typography>
      ...
      {/* Center: Grid Info */}
      <Box
        sx={{ display: 'flex', alignItems: 'center', gap: 3, flex: 1, justifyContent: 'center' }}
      >
        {/* Grid Range */}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        {pnl && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            {pnlValue >= 0 ? (
              <TrendingUp sx={{ color: pnlColor, fontSize: 18 }} />
            ) : (
              <TrendingDown sx={{ color: pnlColor, fontSize: 18 }} />
            )}
