...
import { Box, Paper, Typography, Chip, Tooltip, IconButton, Collapse } from '@mui/material';
import {
...
const TelegramStatusPanel = ({ socket }) => {
  ...
  const StatusIcon = telegramStatus.connected
    ? CheckCircleIcon
    : !telegramStatus.enabled
      ? WarningIcon
      : ErrorIcon;

  const statusColor = telegramStatus.connected
    ? 'success'
    : !telegramStatus.enabled
      ? 'warning'
      : 'error';
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ...
          <Chip
            ...
            sx={{
              ...
              fontSize: '0.7rem',
              fontWeight: 600,
              bgcolor: telegramStatus.connected
                ? 'rgba(76, 175, 80, 0.2)'
                : 'rgba(255, 152, 0, 0.2)',
              color: 'white',
