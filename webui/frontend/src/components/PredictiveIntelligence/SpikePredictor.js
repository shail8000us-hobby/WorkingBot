...
const SpikePredictor = () => {
  ...
  if (loading) {
    return (
      <div
        style={{ padding: '20px', backgroundColor: '#1e1e1e', color: '#fff', borderRadius: '8px' }}
      >
    ...
    >
      ...
      {warning && (
        ...
        >
          ...
          >
            <div>
              Confidence: <strong>{warning.confidence}</strong>
            </div>
            <div>
              Pattern: <strong>{warning.pattern}</strong>
            </div>
            <div>
              Current IV: <strong>{warning.current_iv}</strong>
            </div>
            <div>
              Predicted IV: <strong>{warning.predicted_iv}</strong>
            </div>
          </div>
        </div>
      )}
      ...
      {prediction ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div>
            ...
            <div>
              Confidence:{' '}
              <strong style={{ color: getAlertColor(prediction.alert_level) }}>
                {(prediction.confidence * 100).toFixed(1)}%
              </strong>
            </div>
            <div>
              Time Horizon: <strong>{prediction.minutes_ahead} minutes</strong>
            </div>
            <div>
              Pattern: <strong>{prediction.pattern_match}</strong>
            </div>
          </div>
          ...
          <div>
            <h4>IV Analysis</h4>
            <div>
              Current IV: <strong>{prediction.current_iv}%</strong>
            </div>
            <div>
              Predicted IV: <strong>{prediction.predicted_iv}%</strong>
            </div>
            <div>
              Spike Threshold: <strong>{prediction.spike_threshold}%</strong>
            </div>
            <div>
              Alert Level:{' '}
              <strong style={{ color: getAlertColor(prediction.alert_level) }}>
