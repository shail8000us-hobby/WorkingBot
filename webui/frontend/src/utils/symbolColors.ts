 ...
 * Safe: Pure utility functions, no trading logic
 * 
 * Color Palette:
...
export type SymbolName = 'BTCUSD' | 'ETHUSD' | 'SOLUSD' | 'BNBUSD' | 'XRPUSD' | 'default';

export type BorderSide = 'left' | 'top' | 'right' | 'bottom';

export const SYMBOL_COLORS: Record<SymbolName, SymbolColorScheme> = {
  BTCUSD: { 
    primary: '#3B82F6', 
 ...
 */
export const getSymbolColor = (symbolName: string): SymbolColorScheme => {
  return SYMBOL_COLORS[symbolName as SymbolName] || SYMBOL_COLORS.default;
};

/**
 * Get symbol dot indicator (for logs, badges, etc.)
 */
export const getSymbolDot = (symbolName: string): string => {
 ...
 */
export const getSymbolBorderStyle = (
  symbolName: string, 
 ...
 */
export const getSymbolBadgeProps = (
  symbolName: string, 
