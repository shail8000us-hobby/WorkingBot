...
const FORM_CONFIGS = {
  straddle: {
    fields: [
      { name: 'strike', label: 'Strike Price', type: 'number', required: true, helper: 'Usually ATM' }
    ]
  },
  strangle: {
    fields: [
  ...
  iron_butterfly: {
    fields: [
      ...
      { name: 'wing_width', label: 'Wing Width', type: 'number', required: true, helper: 'Distance to wing strikes' }
    ]
  },
  call_spread: {
    fields: [
...
export default function StrategyForm({ strategyType, template, onSubmit, loading }) {
  ...
  useEffect(() => {
    ...
    const fetchStrikes = async () => {
      ...
      try {
        const res = await fetch(`/api/options-chain/data?underlying=${underlying}&expiry=${expiry}`);
        if (res.ok) {
          const data = await res.json();
          ...
          if (data.atm_strike && !params.strike) {
            setParams(prev => ({ ...prev, strike: data.atm_strike, center_strike: data.atm_strike }));
          }
        }
      } catch (err) {
    ...
    <Box component="form" onSubmit={handleSubmit}>
      ...
      {strikes.length > 0 && (
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
          Available strikes: {strikes.slice(0, 8).map(s => s.toLocaleString()).join(', ')}
