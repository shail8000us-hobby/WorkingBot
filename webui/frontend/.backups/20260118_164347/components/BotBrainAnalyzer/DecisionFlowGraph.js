  ...
  return (
    <Box>
      ...
      <Paper sx={{ p: 2, bgcolor: '#0a0a0a', maxHeight: '75vh', overflow: 'auto' }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
          ...
          {nodes.map((node, idx) => {
            ...
            const color = type === 'start' ? '#4caf50' :
                         type === 'decision' ? '#ff9800' :
                         type === 'action' ? '#00bcd4' :
                         type === 'waiting' ? '#ffeb3b' : '#9c27b0';
            ...
            return (
              <Box key={idx} sx={{ width: '100%', maxWidth: 600, mb: 0.5 }}>
                ...
                >
                  ...
                  <Typography sx={{ color: '#fff', fontSize: '0.7rem', lineHeight: 1.3, whiteSpace: 'pre-line' }}>
                    {label}
                  </Typography>
                </Paper>
                
                {idx < nodes.length - 1 && (
                  <Typography sx={{ color: '#90caf9', fontSize: '1rem', textAlign: 'center', my: 0.3 }}>
                    ↓
                  </Typography>
                )}
              </Box>
        ...
        {/* Legend */}
        <Box sx={{ mt: 3, pt: 2, borderTop: '1px solid rgba(144, 202, 249, 0.2)', textAlign: 'center' }}>
          <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Chip label="🟢 START" size="small" sx={{ bgcolor: '#2e7d32', color: '#fff', fontSize: '0.6rem', height: 20 }} />
            <Chip label="🟠 DECISION" size="small" sx={{ bgcolor: '#e65100', color: '#fff', fontSize: '0.6rem', height: 20 }} />
            <Chip label="🔵 ACTION" size="small" sx={{ bgcolor: '#006064', color: '#fff', fontSize: '0.6rem', height: 20 }} />
            <Chip label="🟡 WAITING" size="small" sx={{ bgcolor: '#424242', color: '#fff', fontSize: '0.6rem', height: 20 }} />
          </Box>
        </Box>
      </Paper>
    </Box>
