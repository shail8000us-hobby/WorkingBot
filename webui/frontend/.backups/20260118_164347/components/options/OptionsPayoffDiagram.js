...
const blackScholesPrice = (S, K, T, r, sigma, type) => {
  ...
  if (sigma <= 0 || S <= 0 || K <= 0) return type === 'call' ? Math.max(0, S - K) : Math.max(0, K - S);
...
const calculateImpliedVolatility = (marketPrice, S, K, T, r, type) => {
  if (T <= 0 || marketPrice <= 0) return 0.80;
...
const OptionsPayoffDiagram = ({ positions, hiddenPositions = [] }) => {
  ...
  const parsedPositions = useMemo(() => {
    ...
    const visiblePositions = positions.filter(
      (p) => !hiddenPositions.includes(p.product_symbol)
    );
    ...
    const parsed = visiblePositions.map(pos => {
      ...
      return { symbol: pos.product_symbol, type: optionType, strike, expiryDate, daysToExpiry, yearsToExpiry, size, entryPrice, markPrice, iv };
    });
    
    // Calculate actual time to nearest expiry (can be fractional days)
    const nearestExpiry = parsed.reduce((min, p) => p.expiryDate < min ? p.expiryDate : min, parsed[0].expiryDate);
    const now = new Date();
  ...
  if (!chartData) {
    return (
      <Paper sx={{ p: 3, textAlign: 'center', bgcolor: 'background.default' }}>
        <Typography color="text.secondary">No visible positions to display payoff diagram</Typography>
      </Paper>
    );
  }
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <Paper sx={{ p: 1.5, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }}>
        <Typography variant="body2" fontWeight="bold">BTC: ${Number(label).toLocaleString()}</Typography>
        <Divider sx={{ my: 0.5 }} />
        <Typography variant="body2" sx={{ color: expiry >= 0 ? '#10b981' : '#ef4444' }}>
          On Expiry: <strong>{expiry >= 0 ? '+' : ''}${expiry.toFixed(2)}</strong>
        </Typography>
        <Typography variant="body2" sx={{ color: '#3b82f6' }}>
          On Target Date: <strong>{target >= 0 ? '+' : ''}${target.toFixed(2)}</strong>
        </Typography>
      </Paper>
    );
  };
  ...
  const dateMarks = [
    { value: 0, label: 'Now' },
    { value: minDaysToExpiry, label: minDaysToExpiry < 1 
  ...
  return (
    <Paper sx={{ p: 2, bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
        <Typography variant="h6" fontWeight="bold">Payoff Graph</Typography>
        
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          {/* Legend */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Box sx={{ width: 24, height: 3, background: 'linear-gradient(90deg, #ef4444 0%, #ef4444 50%, #10b981 50%, #10b981 100%)' }} />
      ...
      <ResponsiveContainer width="100%" height={350}>
        ...
        >
          ...
          <XAxis 
            ...
            stroke="#666"
            tick={{ fontSize: 11 }}
            domain={isZoomed && zoomDomain.left ? [zoomDomain.left, zoomDomain.right] : ['auto', 'auto']}
          ...
          <ReferenceLine 
            ...
            strokeWidth={2}
            strokeDasharray="5 5"
            label={{ value: targetPricePercent === 0 ? 'ATM' : 'Target', position: 'top', fill: '#fbbf24', fontSize: 10 }}
          />
        </ComposedChart>
      </ResponsiveContainer>
