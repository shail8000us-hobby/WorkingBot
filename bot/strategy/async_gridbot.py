...
from bot.strategy.modules.recovery_actions import RecoveryActions
...
class AsyncGridBot:
    ...
    # Guardian state (transitions, missed orders) now owned by GuardianHandler
        ...
        # Phase 7: RecoveryActions module
    ...
    async def seed_missed_grid_levels(self, count: int) -> None:
        """Delegate to GridEngine (moved in P6.7). Kept for external callers."""
        await self.grid_engine.seed_missed_grid_levels(count)
    ...
    async def start(self) -> None:
        ...
        self.health_monitor.set_runtime_refs(
            ...
            _get_last_block_reason=lambda: self.grid_engine._last_block_reason,
            _set_last_block_reason=lambda v: setattr(self.grid_engine, '_last_block_reason', v),
        ...
        # Wire RecoveryActions runtime refs
        self.recovery_actions.set_runtime_refs(
            _get_running=lambda: self._running,
            _get_last_safety_check_time=lambda: self._last_safety_check_time,
    ...
    # _log_detailed_grid_status — MOVED to GridEngine.log_detailed_grid_status() (P6.4)
    ...
    # _fill_polling_fallback_loop — MOVED to RecoveryActions.fill_polling_fallback_loop() (P7.3)
    # _safety_gatekeeper_loop — MOVED to RecoveryActions.safety_gatekeeper_loop() (P7.2)
    # _check_for_unhedged_positions — MOVED to RecoveryActions.check_for_unhedged_positions() (P7.2)
    ...
    # _process_tp_retry_queue — MOVED to RecoveryActions.process_tp_retry_queue() (P7.4)

    # Phase 2: _watchdog_loop MOVED to HealthMonitor.watchdog_loop() (P2.7)
    
    # ========================================================================
    # NOV 20: Reconciliation Action Queue Processor — MOVED to RecoveryActions (P7.5, P7.6)
    # ========================================================================
    # _reconciliation_action_processor — MOVED to RecoveryActions.reconciliation_action_processor() (P7.5)
    # _execute_reconciliation_action — MOVED to RecoveryActions.execute_reconciliation_action() (P7.5)
    # _place_emergency_tp_for_position — MOVED to RecoveryActions.place_emergency_tp_for_position() (P7.6)
