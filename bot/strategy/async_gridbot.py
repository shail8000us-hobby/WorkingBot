...
class AsyncGridBot:
    ...
    async def _fill_multi_step_missed_grids(self) -> None:
        """
        ...
        price_gap = lowest_entry - self.current_price
        ...
        for level in missed_levels:
            ...
            if self.current_price and self.current_price >= level:
                log.info(f"   ⏭️  Skipping ${level:,.0f} — price bounced above it (${self.current_price:,.0f})")
                continue
    ...
    async def _resume_grid_trading(self) -> None:
        """
        ...
        try:
            ...
            if self.mode == "LONG":
                ...
                if next_buy_level and next_buy_level >= self.grid_calc.lower:
                    ...
                    log.info(f"   Current price: ${self.current_price:,.0f}")
