...
```
Grid Bot alone:     BUY @ 60000 → SELL @ 60500 = $5 profit
Grid Bot + SSRFO:   BUY @ 60000 → SELL CE @ 60500 = ~$3 premium
                    → SELL @ 60500 = $5 + $3 = $8 profit (+60% more)
                    → If CE expires worthless = $5 + $3 premium kept
```
...
- **1:1 lot ratio** (1 future lot = 1 CE lot)
...
#### Case 1: Market > entry + 0.5% (e.g., > 60,100)
...
#### Case 2: Market < entry - 200 (e.g., market at 59,800 or below)

```
Market is significantly BELOW entry — the future is underwater.
...
#### Case 3: Market between entry - 500 and entry - 200 (e.g., 59,500 to 59,800)

```
Market is moderately below entry.
ACTION: Sell new CE at strike ~60,000 (ENTRY level, not TP level).
        Closer to ATM = higher premium.
        Resume SSRFO tracking.
RATIONALE: The market is weak. Sell closer-to-ATM CE for higher premium.
           Accept that if market rallies past 60,000, the CE might go ITM.
```

#### Case 4: Market within entry ± 0.5% (e.g., 59,700 to 60,300)
...
```
If grid bot stops → SSRFO pauses new actions, but continues managing existing pairs
If MMM module fails → SSRFO manages orphaned options directly (basic mode)
If options chain unavailable → Skip CE sells, log, retry on next fill
If exchange API rate-limited → Exponential backoff with queue
```
...
├── ssrfo_engine.py             # Core decision logic: what to do on each fill
├── ssrfo_monitor.py            # Background polling thread for EventStore
├── ssrfo_options.py            # Strike selection, expiry selection, CE/PE selling
├── ssrfo_state.py              # Data models: Session, FutureOptionPair, enums
├── ssrfo_storage.py            # SQLite persistence (data/ssrfo_sessions.db)
├── ssrfo_expiry_watcher.py     # CE expiry monitoring + edge case handlers
├── ssrfo_mmm_bridge.py         # MMM session creation/tracking
├── ssrfo_config.py             # Parameter defaults and validation
├── ssrfo_safety.py             # Safety checks, circuit breakers, reconciliation
├── ssrfo_websocket.py          # SocketIO event emitters
├── ssrfo_telegram.py           # Telegram notification helpers
└── tests/
...
|--------|-------------|----------------|--------------|
| `ssrfo_api.py` | ~600 | REST routes, request validation, session CRUD | All modules |
| `ssrfo_engine.py` | ~500 | Fill classification, action determination, orchestration | options, mmm_bridge, state |
| `ssrfo_monitor.py` | ~400 | Background thread, EventStore polling, fill routing | engine, storage, EventStore DB |
| `ssrfo_options.py` | ~450 | Strike/expiry selection, order placement, premium checks | OptionsChainService, place_smart_order |
| `ssrfo_state.py` | ~300 | Dataclasses, enums, session/pair creation helpers | None (pure data) |
| `ssrfo_storage.py` | ~350 | SQLite CRUD, WAL mode, migration support | sqlite3 |
| `ssrfo_expiry_watcher.py` | ~350 | Expiry countdown, case routing, re-entry logic | options, engine |
| `ssrfo_mmm_bridge.py` | ~300 | MMM session creation, import mode, tracking | MMM module internals |
| `ssrfo_config.py` | ~150 | Default params dict, validation, bounds checking | None |
| `ssrfo_safety.py` | ~400 | Max exposure, margin, circuit breaker, reconciliation | Exchange API |
| `ssrfo_websocket.py` | ~150 | SocketIO emit wrappers | flask_socketio |
| `ssrfo_telegram.py` | ~100 | Telegram bot message formatting | telegram helpers |
...
@dataclass
class SSRFOSession:
    session_id: str                    # e.g., "ssrfo-btc-1"
    status: str                        # IDLE, RUNNING, PAUSED, STOPPED
    symbol: str                        # "BTCUSD"
    created_at: str                    # ISO datetime
    started_at: Optional[str]         # When RUNNING started
    stopped_at: Optional[str]         # When STOPPED
    
    # Grid bot link
    grid_config: dict                  # Snapshot of grid config at creation
    grid_step: float                   # 500 for BTC
    lot_size: int                      # 5 for BTC
    eventstore_db_path: str           # "data/bot_events_BTCUSD_LONG.db"
    last_processed_timestamp: float   # For polling cursor
    last_processed_correlation_id: str # Dedup
    
    # SSRFO config
    params: dict                       # From ssrfo_config.py defaults + overrides
    
    # Tracking
    pairs: List[FutureOptionPair]     # All tracked pairs
    spawned_mmm_sessions: List[str]   # MMM session IDs created by this SSRFO
    
    # Stats
    total_pairs_created: int
    total_ce_sold: int
    total_ce_premium_collected: float  # $ total
    total_grid_pnl: float             # $ from TP fills
    total_mmm_pnl: float             # $ from spawned MMM sessions
    total_pnl: float                  # Sum of all
    
    # Safety
    circuit_breaker_active: bool
    consecutive_failures: int
    last_error: Optional[str]
    last_reconciliation: Optional[str] # ISO datetime
```
...
```python
@dataclass
class FutureOptionPair:
    pair_id: str                       # UUID
    session_id: str                    # Parent SSRFO session
    status: str                        # See pair state machine above
    created_at: str                    # ISO datetime
    
    # Future side
    future_entry_price: float          # e.g., 60000
    future_tp_price: float             # e.g., 60500
    future_size: int                   # lots (e.g., 5)
    future_fill_correlation_id: str   # From EventStore, for dedup
    future_fill_time: str              # ISO datetime
    future_side: str                   # "buy" for LONG mode
    
    # CE side
    ce_strike: Optional[float]         # e.g., 60500
    ce_symbol: Optional[str]           # e.g., "C-BTC-60500-210226"
    ce_lots: Optional[int]             # e.g., 5 (1:1 with future)
    ce_premium: Optional[float]        # Premium received per lot (at fill)
    ce_total_premium: Optional[float]  # ce_premium × ce_lots
    ce_order_id: Optional[str]         # Exchange order ID
    ce_fill_time: Optional[str]        # ISO datetime
    ce_expiry_time: Optional[str]      # UTC datetime of CE expiry
    ce_expiry_ddmmyyyy: Optional[str]  # Raw expiry string
    
    # TP fill tracking
    tp_fill_time: Optional[str]        # When the TP order filled
    tp_fill_correlation_id: Optional[str]
    grid_pnl: Optional[float]          # step × contract_mult × lots
    
    # Orphan management
    pe_strike: Optional[float]         # PE sold for strangle
    pe_symbol: Optional[str]
    pe_lots: Optional[int]
    pe_premium: Optional[float]
    pe_order_id: Optional[str]
    mmm_session_id: Optional[str]      # MMM session managing this orphan
    
    # Expiry case tracking
    expiry_case: Optional[str]         # "CASE_1", "CASE_2", "CASE_3", "CASE_4"
    expiry_action_taken: Optional[str] # What was done
    expiry_new_ce_symbol: Optional[str] # If new CE was sold (Case 2/3)
```
...
async def process_tp_fill(session, fill_event):
    ...
    Args:
        session: SSRFOSession
        fill_event: {close_price, entry_price, correlation_id, ...}
    """
    
    # 1. Find matching pair by TP price
    pair = find_pair_by_tp_price(session, fill_event.close_price)
    if not pair:
        log.warning(f"TP fill at {fill_event.close_price} has no matching pair")
        return
    
    # 2. Update pair with TP fill info
    pair.status = "TP_FILLED"
    pair.tp_fill_time = fill_event.timestamp
    pair.tp_fill_correlation_id = fill_event.correlation_id
    pair.grid_pnl = session.grid_step * CONTRACT_MULTIPLIER * session.lot_size
    ...
    pe_strike, pe_symbol, pe_expiry = await select_pe_parameters(
        ...
        tolerance=session.params['strike_tolerance'],
        # Use SAME expiry as the orphaned CE if possible
        preferred_expiry=pair.ce_expiry_ddmmyyyy
    )
    
    # 4. Sell PE
    pe_result = await place_pe_sell(
        symbol=pe_symbol,
    ...
    mmm_session_id = await create_mmm_session_for_orphan(
        ...
        ce_premium=await get_current_premium(pair.ce_symbol),
        pe_symbol=pe_symbol,
        pe_strike=pe_strike,
...
async def process_expiry(session, pair):
    ...
    Cases from notes:
      Case 1: market > entry + 0.5%  → close future
      Case 2: market < entry - 200    → new CE at TP strike
      Case 3: market 59500-59800      → new CE at entry strike
      Case 4: market ≈ entry ± 0.5%   → evaluate premium
    """
    ...
    lower_reentry = session.params['expiry_reentry_pts']  # 200
    lower_moderate = entry - step                         # entry - 500
    
    upper_threshold = entry * (1 + upper_pct / 100)  # e.g., 60300
    lower_threshold = entry - lower_reentry           # e.g., 59800
    
    # Case 1: Market well above entry → close future, take profit
    ...
    # Case 2: Market significantly below → new CE at TP level
    elif market_price < lower_threshold and market_price >= lower_moderate:
    ...
    # Case 3: Market moderately below → new CE at entry level (higher premium)
    elif market_price < lower_moderate:
    ...
    # Case 4: Market near entry → check premium viability
    else:
        pair.expiry_case = "CASE_4"
        premium = await check_ce_premium_at_strike(pair.future_tp_price)
        if premium >= session.params['min_ce_premium']:
            new_ce = await sell_new_ce(pair, target_strike=pair.future_tp_price)
            update_pair_with_new_ce(pair, new_ce)
            pair.status = "ACTIVE"
...
```python
class SSRFOMonitor(threading.Thread):
    """
    ...
    - Thread runs in Flask process (webui-backend PM2)
    """
    
    def __init__(self, session_id: str, session: dict):
        super().__init__(daemon=True, name=f"ssrfo-{session_id}")
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            try:
                loop.run_until_complete(self._poll_cycle())
    ...
    def _read_eventstore(self) -> List[dict]:
        ...
        cursor = conn.execute(
            """SELECT * FROM events 
               WHERE timestamp > ? 
               AND event_type IN ('position_opened', 'position_closed')
...
async def _route_event(self, event: dict):
    ...
    if event_type == 'position_opened':
        ...
        await self.engine.process_entry_fill(self.session, {
        ...
        await self.engine.process_tp_fill(self.session, {
...
| Flask process restart | On `init_ssrfo()`, restore monitors from SSRFO DB (like MMM does). |
...
```python
async def place_ce_sell(symbol: str, lots: int, order_preference: str) -> dict:
    """
    Place a CE sell order using existing options infrastructure.
    
    Uses place_smart_order() from options_control.py which handles:
    - Maker-first: limit at mid-price → wait for fill
    - Maker-only: limit only, no market fallback  
    - Market-only: immediate market order
    """
    from webui.backend.routes.options.options_control import (
        place_smart_order, get_unified_client
    )
...
def build_option_symbol(option_type: str, underlying: str, 
                        strike: float, expiry_ddmmyyyy: str) -> str:
    """
    ...
    ddmmyy = expiry_ddmmyyyy[:2] + expiry_ddmmyyyy[2:4] + expiry_ddmmyyyy[6:8]
...
async def create_mmm_session_for_orphan(
    ...
    Uses internal MMM module functions (same Flask process).
    """
    from webui.backend.routes.mmm.mmm_storage import get_storage
    from webui.backend.routes.mmm.mmm_state import create_session as mmm_create_session
    from webui.backend.routes.mmm.mmm_monitor import start_session_monitor
    
    # 1. Generate session ID
    session_id = generate_mmm_session_id(naming_price)
    # e.g., "mmm-ssrfo-59500-1"
    
    # 2. Create session state with IMPORT mode
    session = mmm_create_session(
        session_id=session_id,
        mode='import',
        params={
            # Conservative params for SSRFO-spawned sessions
            'desired_ce_premium': ce_premium,
            'desired_pe_premium': pe_premium,
            'initial_lots': ce_lots,
            'expiry': expiry,
            'adjustment_interval': 300,     # 5 min
            'min_trigger_move_pct': 5.0,    # 5% trigger (conservative)
            'shift_threshold': 50.0,
            'max_loss_amount': 500.0,       # Per session
            'close_at_premium': 5.0,        # Close at ₹5
            'auto_close_mins': 5,
            'wind_down_hours_before_expiry': 2.0,
            # Mark as SSRFO-spawned for tracking
            '_spawned_by': 'ssrfo',
            '_source_pair_id': pair_id,
        },
        import_data={
            'ce': {
                'strike': ce_strike,
    ...
    # 4. Start the MMM heartbeat monitor
    session['strategy_status'] = 'RUNNING'
    storage.save_session(session)
    start_session_monitor(session_id, session)
    
    log.info(f"Created MMM session {session_id} for orphan: "
...
```python
def generate_mmm_session_id(naming_price: float) -> str:
    """
    Generate MMM session ID for SSRFO-spawned sessions.
    
    Format: mmm-ssrfo-{price}-{sequence}
    Examples: mmm-ssrfo-59500-1, mmm-ssrfo-60000-1
    """
    from webui.backend.routes.mmm.mmm_storage import get_storage
    
    base = f"mmm-ssrfo-{int(naming_price)}"
    storage = get_storage()
    existing = storage.list_sessions()
    
    # Find next sequence number
    seq = 1
    while f"{base}-{seq}" in [s.get('session_id') for s in existing]:
        seq += 1
...
18. SSRFO creates MMM session: "mmm-ssrfo-81500-1"
    - CE: -5 @ 82,000
    - PE: -5 @ 81,500
19. MMM heartbeat starts, manages the strangle
20. Telegram: "💰 TP hit @ 82,000 | Grid P&L: $2.50 | MMM: mmm-ssrfo-81500-1"
...
25. SSRFO creates MMM session: "mmm-ssrfo-82000-1"
26. Telegram: "💰 TP hit @ 82,500 | Grid P&L: $2.50 | MMM: mmm-ssrfo-82000-1"
...
CE Premium:      $4.20×5 + $3.80×5 = $40.00 collected
MMM Session PnL: Depends on MMM management (~$5-20 estimated)
─────────────────────────────────
Estimated Total:  $50 - $65 vs $5.00 grid-only (10-13x improvement)
```
