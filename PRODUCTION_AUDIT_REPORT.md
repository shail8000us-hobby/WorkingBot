...
### A2. Multi-Order Grid (Keep 2-3 Pending Orders)
...
#### C1. `self.lot` AttributeError — Will Crash on Missed Order Retry
...
#### C2. Fill ID Deduplication Format Mismatch — Risk of Double-Processing or Missed Fills
...
#### C3. Guardian Risk Engine Thread Can Die Silently
...
#### C4. WebSocket `_ws_is_alive()` Returns True for Unknown Types
...
#### C5. Fill Polling Fallback Has 120s Blind Spot at Startup
...
#### H1. PnL Database Path Mismatch Between Guardian and Bot
...
#### H2. Orphan TP → Entry Price Heuristic Is Fragile  
...
#### H3. Event Store Query Limit Too Low for Orphan Recovery
...
#### H4. WebSocket Message Queue Can Block Producer
...
#### H5. Unified API Client WebSocket Health Monitor Can't Be Stopped
...
#### H6. `_handle_position_update()` Is a No-Op
...
#### M1. Heartbeat Interval Not Using Config Value
...
#### M2. ANSI Escape Codes in Log Output
...
#### M3. Rate Limiter Not Shared Across Components
...
#### M4. Dual Ping Mechanism in WebSocket Manager
...
#### M5. Config Loader Uses Relative Paths
...
#### M6. `get_api_credentials()` Returns None Silently
...
#### M7. Position Validation Uses Entry Price Matching
...
#### M8. `sys.setrecursionlimit(5000)` in Guardian
...
#### M9. No Timeout on `exchange.fetch_balance()` at Guardian Startup
...
#### M10. Guardian Config Hot-Reload Is Broken
...
#### L1. Backup/Legacy Files in Production Directory
...
#### L2. No File Locking on Config Read/Write
...
#### L3. FillMonitor `_cleanup_old_orders()` Uses Hardcoded 3600s
...
#### L4. `monitoring_loop()` Duplicate Writes
**File:** `bot/strategy/async_gridbot.py`  
...
#### L5. Background Task Crash Silently Discarded in WebSocket Manager
...
#### L6. WebSocket Circuit Breaker 300s Blackout
...
- [ ] **C1**: Replace `self.lot` with `self.lot_size` at lines 1012, 1024 of `async_gridbot.py` — current code will crash
- [ ] **C2**: Unify fill ID format across WebSocket, FillMonitor, and polling fallback
- [ ] **C4**: Change `_ws_is_alive()` fallback to `return False` instead of `True`
- [ ] **C5**: Reduce fill polling initial delay from 120s to 30s

### Should Fix (High Priority)

- [ ] **C3**: Add Guardian risk engine thread health monitoring
- [ ] **H1**: Unify PnL database path (Guardian and bot use same DB)
- [ ] **H4**: Use `put_nowait()` with overflow handling in WebSocket message queue
- [ ] **H6**: Implement `_handle_position_update()` to detect exchange-initiated liquidations
- [ ] **H7**: Remove double sleep in FillMonitor
